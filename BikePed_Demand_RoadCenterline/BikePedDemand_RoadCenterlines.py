...
def getFieldNames(fc3):
    keepfields = ["PopDens2018", "JobDens2018", "lumixscore", "K12Schoolsdist", "ParksTrailheadsdist", "collegedist",
                  "RetailCentersdist", "busdist", "RailStationsdist",
                  "B01001_calc_pctDependE", "B17020_calc_pctPovE", "B08201_calc_pctNoVehE", "BlockLength",
                  "MeanBikeRating", "MeanPedRating", "area_sqm",
                  "Shape_Area", "OBJECTID", "Shape_Length", "Shape", "STATUS", "CARTOCODE", "FULLNAME", "UNIQUE_ID", "COUNTY_L", "COUNTY_R",
                  "INCMUNI_L", "INCMUNI_R", "UNINCCOM_L", "UNINCCOM_R", "UTRANS_NOTES"]
...
arcpy.CalculateGeometryAttributes_management("RoadsCopy", [["BlockLength", "LENGTH"]], "FEET_US", '')
...
arcpy.SpatialJoin_analysis(roads, "PopTAZ", "RoadsJoin1", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings, "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin1")
...
arcpy.SpatialJoin_analysis("RoadsJoin1", "JobsTAZ", "RoadsJoin2", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings,
                           "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin2")
...
arcpy.SpatialJoin_analysis("RoadsJoin2", "LandUseMixIndex", "RoadsJoin3", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings,
                           "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin3")
...
arcpy.SpatialJoin_analysis("RoadsJoin3", "K12Schoolsbuffer", "RoadsJoin4", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings,
                           "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin4")
...
arcpy.SpatialJoin_analysis("RoadsJoin4", "ParksTrailheadsbuffer", "RoadsJoin5", "JOIN_ONE_TO_ONE", "KEEP_ALL",
                           fieldmappings, "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin5")
...
arcpy.SpatialJoin_analysis("RoadsJoin5", "CollegesBuffer", "RoadsJoin6", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings,
                           "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin6")
...
arcpy.SpatialJoin_analysis("RoadsJoin6", "RetailCentersbuffer", "RoadsJoin7", "JOIN_ONE_TO_ONE", "KEEP_ALL",
                           fieldmappings, "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin7")
...
arcpy.SpatialJoin_analysis("RoadsJoin7", "BusStopsBuffer", "RoadsJoin8", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings,
                           "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin8")
...
arcpy.SpatialJoin_analysis("RoadsJoin8", "RailStationsbuffer", "RoadsJoin9", "JOIN_ONE_TO_ONE", "KEEP_ALL",
                           fieldmappings, "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin9")
...
arcpy.SpatialJoin_analysis("RoadsJoin9", "Age_ACS_Tract", "RoadsJoin10", "JOIN_ONE_TO_ONE", "KEEP_ALL", fieldmappings,
                           "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin10")
...
arcpy.SpatialJoin_analysis("RoadsJoin10", "Poverty_ACS_Tract", "RoadsJoin11", "JOIN_ONE_TO_ONE", "KEEP_ALL",
                           fieldmappings, "INTERSECT")

fieldmappings = arcpy.FieldMappings()
fieldmappings.addTable("RoadsJoin11")
...
arcpy.SpatialJoin_analysis("RoadsJoin11", "Vehicles_ACS_Tract", "DemandScore", "JOIN_ONE_TO_ONE", "KEEP_ALL",
                           fieldmappings, "INTERSECT")
...
removefcs = ["RoadsJoin1", "RoadsJoin2", "RoadsJoin3", "RoadsJoin4", "RoadsJoin5", "RoadsJoin6", "RoadsJoin7",
             "RoadsJoin8", "RoadsJoin9",
             "RoadsJoin10", "RoadsJoin11", "RoadsCopy"]
for f in removefcs:
...
joinfc = "DemandScore"
checkfields = ["PopDens2018", "JobDens2018", "lumixscore", "K12Schoolsdist", "ParksTrailheadsdist", "collegedist",
               "RetailCentersdist", "busdist", "RailStationsdist",
               "B01001_calc_pctDependE", "B17020_calc_pctPovE", "B08201_calc_pctNoVehE", "BlockLength",
               "MeanBikeRating", "MeanPedRating"]
...
arcpy.management.CalculateField(joinfc, "BikeDemandScore",
                                "((!popdensrating!*9)+(!jobdensrating!*11)+(!lumixrating!*10)+(!schoolrating!*10)+(!parkrating!*5)+(!collegerating!*10)+(!retailrating!*11)+(!busrating!*3)+(!railrating!*11)+(!agerating!*3)+(!novehrating!*4)+(!povrating!*6)+(!blocklengthrating!*2)+(!MeanBikeRating!*5))/100",
                                "PYTHON3", '', "DOUBLE")
arcpy.management.CalculateField(joinfc, "PedDemandScore",
                                "((!popdensrating!*9)+(!jobdensrating!*9)+(!lumixrating!*11)+(!schoolrating!*9)+(!parkrating!*5)+(!collegerating!*9)+(!retailrating!*12)+(!busrating!*4)+(!railrating!*10)+(!agerating!*3)+(!novehrating!*4)+(!povrating!*5)+(!blocklengthrating!*5)+(!MeanPedRating!*5))/100",
                                "PYTHON3", '', "DOUBLE")

# Clean up - delete buffer feature classes
print("Deleting buffers...")
removebuffers = ["RailStationsbuffer", "K12Schoolsbuffer", "ParksTrailheadsbuffer", "RetailCentersbuffer",
                 "CollegesBuffer", "BusStopsBuffer"]
